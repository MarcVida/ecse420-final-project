...
float get_MSE(char* input_filename_1, char* input_filename_2)
{
  ...
  if(error1) printf("error1 %u: %s\n", error1, lodepng_error_text(error1));
  if(error2) printf("error2 %u: %s\n", error2, lodepng_error_text(error2));
  if(width1 != width2) printf("images do not have same width: width1: %d vs width 2: %d\n", width1, width2);
  if(height1 != height2) printf("images do not have same height: height1: %d vs height 2 : %d\n", height1, height2);
